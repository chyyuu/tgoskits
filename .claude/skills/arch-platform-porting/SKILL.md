...
- **RISC-V `std-compat` startup**: initialize the primary TLS area immediately after the global allocator and before platform device probing when `tls` is enabled without `multitask`; Rust std's panic runtime may access `tp` while reporting a probe failure. A QEMU `virt` configuration that enables `irq` must also enable `paging`, because FDT-discovered PLIC registers are mapped through `ax-mm::iomap()`.
